...
def selected_option(option, file_dropdown):
    if option == "Open":
...
def main():
    ...
    file_dropdown_options = ("Save", "Open")
    file_dropdown.set("File")  # Default text

    file_dropdown_options = tk.OptionMenu(navbar, file_dropdown, *file_dropdown_options, command=lambda option: selected_option(option, file_dropdown))
